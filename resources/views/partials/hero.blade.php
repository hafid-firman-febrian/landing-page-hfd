<section id="hero" class="relative overflow-hidden">
    <div class="pointer-events-none absolute -top-40 -right-40 h-96 w-96 rounded-full bg-brand-600/20 blur-3xl"></div>
    <div class="mx-auto max-w-6xl px-4 py-20 md:py-28 grid md:grid-cols-2 gap-12 items-center relative">
        <div>
            @if(!empty($settings['hero_badge']))
                <span class="inline-block rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs text-brand-50 mb-6">
                    {{ $settings['hero_badge'] }}
                </span>
            @endif
            ...
            <div class="flex flex-wrap gap-4">
                @if(!empty($settings['hero_cta_primary_label']))
                    <a href="{{ $settings['hero_cta_primary_url'] ?? '#cta' }}"
                       class="rounded-full bg-brand-600 px-6 py-3 font-semibold text-white hover:bg-brand-700 transition">
                ...
                @if(!empty($settings['hero_cta_secondary_label']))
                    <a href="{{ $settings['hero_cta_secondary_url'] ?? '#portfolio' }}"
                       class="rounded-full border border-white/20 px-6 py-3 text-white hover:bg-white/5 transition">
        ...
        @else
            <div class="justify-self-center hidden md:block">
                <div class="w-80 h-80 rounded-3xl bg-gradient-to-br from-brand-500 to-brand-700 shadow-2xl flex items-center justify-center text-7xl">
                    🚀
                </div>
            </div>
        @endif
